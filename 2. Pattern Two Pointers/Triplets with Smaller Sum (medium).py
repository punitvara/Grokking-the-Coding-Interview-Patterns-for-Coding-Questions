...
class Solution:
    def countTriplets(self, arr, n, sum):
        ...
        for i in range(length -2):
            ...
            right = length - 1
            while (left < right):
                triplet_sum = arr[left] + arr[i] + arr[right]

                # if triplet sum is found than target sum then there will be right-left elements for which
                # triplet sum will be less than target sum
                if triplet_sum < sum:
                    count += right - left
                    left += 1
                # If triplet sum is more than target sum or equal, try to reduce sum by reducing right index
                else:
                    right -= 1
        return count
