...
def binary_search(arrayreader, key, start, end):
    mid = start + (end - start) // 2
    if key > arrayreader.get(mid):
        start = mid + 1
    elif key < arrayreader.get(mid)
        end = mid - 1
    else:
        return mid
    return -1
    
def find_key(self, arrreader, key):
    start, end = 0, 1
    while arrayreader.get(end) < k:
        newstart = end + 1
        end += (end - start + 1) * 2

        start = newstart
    return self.binary_search(arrayreader, key, start, end)
