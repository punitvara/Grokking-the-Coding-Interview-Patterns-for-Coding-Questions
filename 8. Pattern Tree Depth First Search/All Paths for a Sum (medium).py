...
'''
class Solution:
    def dfs(self, node: TreeNode, s: int, l1, l2):
        if not node:
            return []
        #Add current node
        l1.append(node.val)

        #if node left and right is null and sum = current node val then add currentlist l1 to l2
        if node.val == s and node.left == None and node.right == None:
            l2.append(list(l1))

        self.dfs(node.left, s - node.val, l1, l2)
        self.dfs(node.right, s - node.val, l1, l2)
        #to backtrack delete last element.otherwise while backtracking all elements will be present in L1.
        del l1[-1]
        return l2
    def pathSum(self, root: TreeNode, targetSum: int) -> List[List[int]]:
        self.l2 = []
        self.l1 = []
        return self.dfs(root, targetSum, self.l1, self.l2)
#answer
class TreeNode:
