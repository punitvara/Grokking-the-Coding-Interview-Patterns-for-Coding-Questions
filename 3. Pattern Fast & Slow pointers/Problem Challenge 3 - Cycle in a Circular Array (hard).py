...
# my code
class Solution:
    def next_iter_index(self, nums: List[int], is_forward, current_index):
    ...
    def circularArrayLoop(self, nums: List[int]) -> bool:
        ...
        for i in range(len(nums)):
            fast, slow = i, i
            is_forward = nums[i] >= 0 #If we are moving forward

            # if slow or fast becomes '-1' this means we can't find cycle for this number
            while True:
                # move one step for slow pointer
                slow = self.next_iter_index(nums, is_forward, slow)

                # move one step for fast pointer
                fast = self.next_iter_index(nums, is_forward, fast)

                # move another step only if direction is not reversed
                if fast != -1:
                    fast = self.next_iter_index(nums, is_forward, fast)
