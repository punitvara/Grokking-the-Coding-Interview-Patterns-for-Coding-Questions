...
Output: [1, 2, 3, 4, 5, 6]
'''
# Cyclic sort is very useful when minimum writes are needed to sort an array. Also very easy to get swap number with count.
...
def cyclic_sort(nums):
  ...
  while i < len(nums):
    ...
    if nums[i] != nums[next_index]:
      nums[i], nums[next_index] = nums[next_index], nums[i] #swap
    else:
      i += 1
  return nums
