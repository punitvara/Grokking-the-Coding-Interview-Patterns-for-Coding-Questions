...
class Solution:
    def lengthOfLongestSubstring(self, s: str) -> int:
        ...
        for win_end in range(len(s)):
            if s[win_end] not in basket:
                basket[s[win_end]] = 1
            else:
                basket[s[win_end]] += 1

            while sum(basket.values()) > len(basket) :
                if basket[s[win_start]] >=2:
                    basket[s[win_start]] = basket[s[win_start]] - 1
                else:
                    del basket[s[win_start]]
                win_start += 1
            sum_val = sum(basket.values())
            if len(basket) == sum_val:
                max_length = max(max_length, sum_val)
        return max_length
